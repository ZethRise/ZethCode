<#
.SYNOPSIS
    ZethCode installer for Windows.
.DESCRIPTION
    Downloads and installs ZethCode to $env:USERPROFILE\.zethcode\bin,
...
.LINK
    https://github.com/ZethRise/ZethCode
.EXAMPLE
    irm https://raw.githubusercontent.com/ZethRise/ZethCode/main/scripts/install.ps1 | iex
.EXAMPLE
    $env:VERSION = "0.1.0"; irm https://raw.githubusercontent.com/ZethRise/ZethCode/main/scripts/install.ps1 | iex
#>
param(
    [String] $Version,
    [Switch] $NoModifyPath
...
$Arch = if ($env:PROCESSOR_ARCHITECTURE -eq "ARM64") { "arm64" }
    ...
    else { Write-Err "ZethCode requires a 64-bit operating system." }
...
$FdsBase = if ($env:ZETH_FDS_BASE) { $env:ZETH_FDS_BASE.TrimEnd('/') } else {
    "https://zethcode.cnbj1.mi-fds.com/zethcode/zethcode"
}
...
# Warn about old versions that don't support zeth upgrade via install.ps1
...
if ($MajMin -match '^\d+\.\d+\.\d+$') {
    ...
    if ($semver -le 104) {
        Write-Host ""
        Write-Host "WARNING: Installing v$Version via install.ps1 will cause 'zeth upgrade' to not function properly." -ForegroundColor Yellow
...
$Staging = $false
if ($env:ZETHCODE_INSTALL_DIR) {
    $InstallDir = $env:ZETHCODE_INSTALL_DIR
    $Staging = $true
} else {
    $InstallDir = Join-Path $env:USERPROFILE ".zethcode\bin"
}

if (-not $Staging) {
    $Existing = Get-Command zeth -ErrorAction SilentlyContinue
    if ($Existing) {
        $InstalledVersion = & zeth --version 2>$null
        if ($InstalledVersion -eq $Version) {
            Write-Host "ZethCode v$Version is already installed." -ForegroundColor DarkGray
            Exit-Install 0
        }
...
$BinName = if (Test-Path (Join-Path $TmpDir "zeth.exe")) { "zeth.exe" } else { "zeth" }
try {
    Move-Item -Path (Join-Path $TmpDir $BinName) -Destination (Join-Path $InstallDir "zeth.exe") -Force -ErrorAction Stop
} catch {
    Remove-Item -Recurse -Force $TmpDir -ErrorAction SilentlyContinue
    Write-Err "Failed to install binary. If ZethCode is currently running, please close it and retry.`n$($_.Exception.Message)"
}
Remove-Item -Recurse -Force $TmpDir -ErrorAction SilentlyContinue
...
Write-Host "https://github.com/ZethRise/ZethCode#readme"
Write-Host ""
