...
const dir = path.join(os.tmpdir(), "zethcode-test-data-" + process.pid)
...
const fixtureRoot = path.join(process.cwd(), ".zethcode-test-fixtures-" + process.pid)
await fs.mkdir(fixtureRoot, { recursive: true })
process.env["ZETHCODE_TEST_TMPDIR_ROOT"] = fixtureRoot
afterAll(async () => {
...
process.env["ZETHCODE_MODELS_PATH"] = path.join(import.meta.dir, "tool", "fixtures", "models-api.json")
...
process.env["ZETHCODE_TEST_MANAGED_CONFIG_DIR"] = testManagedConfigDir
process.env["ZETHCODE_DISABLE_DEFAULT_PLUGINS"] = "true"

// Write the cache version file to prevent global/index.ts from clearing the cache
const cacheDir = path.join(dir, "cache", "zethcode")
...
delete process.env["ZETHCODE_SERVER_PASSWORD"]
delete process.env["ZETHCODE_SERVER_USERNAME"]
delete process.env["ZETHCODE_HOME"]

// Use in-memory sqlite
process.env["ZETHCODE_DB"] = ":memory:"
...
// ZETHCODE_EXPERIMENTAL_ORCHESTRATOR; the orchestrator test suites exercise the
// feature, so enable it here (Flag is read once at import — must be set first).
process.env["ZETHCODE_EXPERIMENTAL_ORCHESTRATOR"] = "true"
