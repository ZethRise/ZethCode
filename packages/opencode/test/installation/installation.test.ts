...
describe("installation", () => {
  describe("method", () => {
    test("detects npm when @zethrise/cli is in npm list output", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "npm" && args.includes("-g")) return "@zethrise/cli@1.0.0"
          return ""
        },
      )
    ...
    test("detects pnpm when @zethrise/cli is in pnpm list output", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "pnpm" && args.includes("-g")) return "@zethrise/cli@1.0.0"
          return ""
        },
      )
    ...
    test("detects bun when @zethrise/cli is in bun pm ls output", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "bun" && args.includes("-g")) return "@zethrise/cli@1.0.0"
          return ""
        },
      )
    ...
    test("returns unknown when no package manager has @zethrise/cli", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        () => "",
      )
  ...
  describe("latest", () => {
    test("reads version from npm registry for npm method", async () => {
      const layer = testLayer(
        (req) => {
          expect(req.url).toContain(encodeURIComponent("@zethrise/cli"))
    ...
    test("resolves version from FDS for curl method", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "curl" && args.includes("https://zethcode.cnbj1.mi-fds.com/zethcode/zethcode/releases/latest"))
            return "v0.1.1\n"
          return ""
        },
      )
  ...
  describe("upgrade", () => {
    test("runs npm install with correct package", async () => {
      ...
      expect(capturedArgs).toContain("@zethrise/cli@2.0.0")
    })
