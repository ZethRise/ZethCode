...
describe("postinstall", () => {
  ...
  test("creates .zethcode binary cache from platform package", async () => {
    ...
    const cached = path.join(binDir, ".zethcode")
  ...
  test("prints migration notice to stdout", async () => {
    ...
    expect(stdout).toContain("Recommended: install ZethCode natively for a better")
  ...
  test("exits with error when binary package is missing", async () => {
    ...
    expect(result.stdout.toString()).toContain("Recommended: install ZethCode natively")
  })
