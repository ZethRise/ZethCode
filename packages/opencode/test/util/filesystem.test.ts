...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "zethcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "zethcode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "zethcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "zethcode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["zethcode.json", "zethcode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "zethcode.json"),
        path.join(tmp.path, "zethcode.jsonc"),
        path.join(project, "zethcode.json"),
        path.join(project, "zethcode.jsonc"),
      ])
    })
  })
