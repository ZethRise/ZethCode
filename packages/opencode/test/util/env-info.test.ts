...
describe("util.env-info", () => {
  test("returns host, user, runtime, and zethcode metadata", async () => {
    ...
    expect(info.zethcode.version).toBeTruthy()
    expect(info.zethcode.channel).toBeTruthy()
    expect(info.zethcode.installation_id).toMatch(
