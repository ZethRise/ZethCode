...
import type { PluginInput } from "@zethrise/plugin"
...
describe("MimoAuthPlugin", () => {
  ...
  describe("authorize", () => {
    test("returns url with pk, redirect_uri, kn, key_name params", async () => {
      ...
      expect(url.searchParams.get("kn")).toBe("zethcode")
      expect(url.searchParams.get("key_name")).toMatch(/^zeth-code-cli-key-/)
  ...
  describe("chat.headers hook", () => {
    test("adds X-Mimo-Source header for mimo provider", async () => {
      ...
      expect(output.headers["X-Mimo-Source"]).toBe("zethcode-cli")
    })
