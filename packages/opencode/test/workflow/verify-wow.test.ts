...
// the live router + a real key in ~/.config/zethcode/zethcode.json). Run with:
...
function realConfig() {
  const home = path.join(os.homedir(), ".config", "zethcode", "zethcode.json")
      ...
      provideTmpdirInstance(
        (dir) =>
          Effect.gen(function* () {
            ...
            // Two saved workflows on disk, resolved by name from .zethcode/workflows.
            const wfDir = path.join(dir, ".zethcode", "workflows")
