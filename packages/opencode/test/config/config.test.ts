...
import { EffectFlock } from "@zethrise/shared/util/effect-flock"

import { Instance } from "../../src/project/instance"
import { Auth } from "../../src/auth"
import { Account } from "../../src/account/account"
import { AccessToken, AccountID, OrgID } from "../../src/account/schema"
import { AppFileSystem } from "@zethrise/shared/filesystem"
...
const managedConfigDir = process.env.ZETHCODE_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "zethcode.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "zethcode.json") {
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "zethcode.json"))
...
test("migrates old opencode.ai $schema URL to mimo.xiaomi.com", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      await load()
      const content = await Filesystem.readText(path.join(tmp.path, "zethcode.json"))
      expect(content).toContain('"$schema": "https://mimo.xiaomi.com/zethcode/config.json"')
...
test("preserves JSONC comments when injecting $schema", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      await load()
      const content = await Filesystem.readText(path.join(tmp.path, "zethcode.jsonc"))
      expect(content).toContain("$schema")
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["ZETHCODE_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { opencode: { options: { apiKey: "{env:ZETHCODE_CONSOLE_TOKEN}" } } },
        }),
      ),
  ...
  try {
    ...
    if (originalControlToken !== undefined) {
      process.env["ZETHCODE_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["ZETHCODE_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "zethcode.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .zethcode directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".zethcode")
...
test("loads agents from .zethcode/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".zethcode")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .zethcode/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".zethcode")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .zethcode/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".zethcode")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("zethcode command overrides .claude command on name collision", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(
...
from claude`,
      ...
      await Filesystem.write(
        path.join(dir, ".zethcode", "command", "dup.md"),
        `---
description: zethcode version
---
from zethcode`,
      )
    },
...
test("does not try to install dependencies in read-only ZETHCODE_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.ZETHCODE_CONFIG_DIR
  process.env.ZETHCODE_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.ZETHCODE_CONFIG_DIR
    else process.env.ZETHCODE_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable ZETHCODE_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .zethcode config
      const projectDir = path.join(dir, "project")
      const opencodeDir = path.join(projectDir, ".zethcode")
      await fs.mkdir(opencodeDir, { recursive: true })
      ...
      // Local .zethcode config with different plugins
      await Filesystem.write(
        path.join(opencodeDir, "zethcode.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .zethcode config with some overlapping plugins
      await Filesystem.write(
        path.join(opencodeDir, "zethcode.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".zethcode")
...
// Note: preload.ts sets ZETHCODE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .zethcode config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .zethcode directory config enables it
      const opencodeDir = path.join(dir, ".zethcode")
      await fs.mkdir(opencodeDir, { recursive: true })
      await Filesystem.write(
        path.join(opencodeDir, "zethcode.json"),
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "zethcode.json")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.zethcode/plugin/oh-my-opencode.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.zethcode/plugin/demo.ts", "file:///project/.zethcode/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.zethcode/plugin/demo.ts"])
  })
...
describe("ZETHCODE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["ZETHCODE_DISABLE_PROJECT_CONFIG"]
    process.env["ZETHCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["ZETHCODE_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["ZETHCODE_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .zethcode/ directories when flag is set", async () => {
    const originalEnv = process.env["ZETHCODE_DISABLE_PROJECT_CONFIG"]
    process.env["ZETHCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .zethcode directory with a command
          const opencodeDir = path.join(dir, ".zethcode", "command")
          await fs.mkdir(opencodeDir, { recursive: true })
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          // Project .zethcode should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["ZETHCODE_DISABLE_PROJECT_CONFIG"]
    ...
    try {
      ...
      if (originalDisable === undefined) {
        ...
        process.env["ZETHCODE_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["ZETHCODE_CONFIG_DIR"]
      } else {
        process.env["ZETHCODE_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("ZETHCODE_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["ZETHCODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["ZETHCODE_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["ZETHCODE_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["ZETHCODE_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from ZETHCODE_CONFIG_DIR, not project
...
describe("ZETHCODE_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in ZETHCODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["ZETHCODE_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["ZETHCODE_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://opencode.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["ZETHCODE_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["ZETHCODE_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in ZETHCODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["ZETHCODE_CONFIG_CONTENT"]
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          ...
          PayloadIdentifier: "ai.zethcode.managed.test",
          PayloadType: "ai.zethcode.managed",
