...
afterEach(async () => {
  delete process.env.ZETHCODE_CONFIG
  delete process.env.ZETHCODE_TUI_CONFIG
  await fs.rm(path.join(Global.Path.config, "zethcode.json"), { force: true }).catch(() => {})
  await fs.rm(path.join(Global.Path.config, "zethcode.jsonc"), { force: true }).catch(() => {})
...
test("keeps server and tui plugin merge semantics aligned", async () => {
  await using tmp = await tmpdir({ outsideGit: true,
    init: async (dir) => {
      const local = path.join(dir, ".zethcode")
      await fs.mkdir(local, { recursive: true })

      await Bun.write(
        path.join(Global.Path.config, "zethcode.json"),
        JSON.stringify(
          {
...
test("loads tui config with the same precedence order as server config paths", async () => {
  await using tmp = await tmpdir({ outsideGit: true,
    init: async (dir) => {
      ...
      await fs.mkdir(path.join(dir, ".zethcode"), { recursive: true })
      await Bun.write(
        path.join(dir, ".zethcode", "tui.json"),
...
test("migrates tui-specific keys from zethcode.json when tui.json does not exist", async () => {
  await using tmp = await tmpdir({ outsideGit: true,
    init: async (dir) => {
      await Bun.write(
  ...
  const server = JSON.parse(await Filesystem.readText(path.join(tmp.path, "zethcode.json")))
  expect(server.theme).toBeUndefined()
  expect(server.keybinds).toBeUndefined()
  expect(server.tui).toBeUndefined()
  expect(await Filesystem.exists(path.join(tmp.path, "zethcode.json.tui-migration.bak"))).toBe(true)
...
test("skips migration when zethcode.jsonc is syntactically invalid", async () => {
  await using tmp = await tmpdir({ outsideGit: true,
    init: async (dir) => {
      await Bun.write(
  ...
  expect(await Filesystem.exists(path.join(tmp.path, "zethcode.jsonc.tui-migration.bak"))).toBe(false)
  const source = await Filesystem.readText(path.join(tmp.path, "zethcode.jsonc"))
...
test("skips migration when tui.json already exists", async () => {
  await using tmp = await tmpdir({ outsideGit: true,
    init: async (dir) => {
      await Bun.write(path.join(dir, "zethcode.json"), JSON.stringify({ theme: "legacy" }, null, 2))
  ...
  expect(await Filesystem.exists(path.join(tmp.path, "zethcode.json.tui-migration.bak"))).toBe(false)
})
...
test.skip("continues loading tui config when legacy source cannot be stripped", async () => {
  await using tmp = await tmpdir({ outsideGit: true,
    init: async (dir) => {
      await Bun.write(path.join(dir, "zethcode.json"), JSON.stringify({ theme: "readonly-theme" }, null, 2))
    },
  })

  const source = path.join(tmp.path, "zethcode.json")
...
test("migration backup preserves JSONC comments", async () => {
  ...
  const backup = await Filesystem.readText(path.join(tmp.path, "zethcode.jsonc.tui-migration.bak"))
...
test("migrates legacy tui keys across multiple zethcode.json levels", async () => {
  await using tmp = await tmpdir({ outsideGit: true,
    init: async (dir) => {
      const nested = path.join(dir, "apps", "client")
      await fs.mkdir(nested, { recursive: true })
      await Bun.write(path.join(dir, "zethcode.json"), JSON.stringify({ theme: "root-theme" }, null, 2))
      await Bun.write(path.join(nested, "zethcode.json"), JSON.stringify({ theme: "nested-theme" }, null, 2))
    },
  })
...
test("project config takes precedence over ZETHCODE_TUI_CONFIG (matches ZETHCODE_CONFIG)", async () => {
  await using tmp = await tmpdir({ outsideGit: true,
    init: async (dir) => {
      ...
      process.env.ZETHCODE_TUI_CONFIG = custom
    },
  })
...
test("ZETHCODE_TUI_CONFIG provides settings when no project config exists", async () => {
  await using tmp = await tmpdir({ outsideGit: true,
    init: async (dir) => {
      const custom = path.join(dir, "custom-tui.json")
...
test("does not derive tui path from ZETHCODE_CONFIG", async () => {
  await using tmp = await tmpdir({ outsideGit: true,
    init: async (dir) => {
      const customDir = path.join(dir, "custom")
      await fs.mkdir(customDir, { recursive: true })
      await Bun.write(path.join(customDir, "zethcode.json"), JSON.stringify({ model: "test/model" }))
      await Bun.write(path.join(customDir, "tui.json"), JSON.stringify({ theme: "should-not-load" }))
      process.env.ZETHCODE_CONFIG = path.join(customDir, "zethcode.json")
    },
  })
  const config = await getTuiConfig(tmp.path)
  expect(config.theme).toBeUndefined()
...
test("loads .zethcode/tui.json", async () => {
  await using tmp = await tmpdir({ outsideGit: true,
    init: async (dir) => {
      await fs.mkdir(path.join(dir, ".zethcode"), { recursive: true })
      await Bun.write(path.join(dir, ".zethcode", "tui.json"), JSON.stringify({ diff_style: "stacked" }, null, 2))
    },
  })
  const config = await getTuiConfig(tmp.path)
  expect(config.diff_style).toBe("stacked")
...
test("injects $schema when missing from tui.json", async () => {
  ...
  await getTuiConfig(tmp.path)
  const text = await Filesystem.readText(path.join(tmp.path, "tui.json"))
  expect(text).toContain('"$schema": "https://mimo.xiaomi.com/zethcode/tui.json"')
