...
test("agentrouter provider loads from fallback token env", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "zethcode.json"),
  ...
  await Instance.provide({
    directory: tmp.path,
    init: async () => {
      set("AGENT_ROUTER_TOKEN", "test-agentrouter-key")
    },
    fn: async () => {
      const providers = await list()
      const provider = providers[ProviderID.make("agentrouter")]
      expect(provider).toBeDefined()
      expect(provider.name).toBe("AgentRouter")
      expect(provider.options.baseURL).toBe("https://agentrouter.org/v1")
      expect(provider.options.apiKey).toBe("test-agentrouter-key")
      expect(provider.models["gpt-5.5"]).toBeDefined()
      expect(provider.models["gpt-5.5"].api.npm).toBe("@ai-sdk/openai-compatible")
      expect(provider.models["gpt-5.5"].api.url).toBe("https://agentrouter.org/v1")
    },
  })
})
...
test("agentrouter provider loads from config apiKey", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(provider).toBeDefined()
      expect(provider.options.baseURL).toBe("https://agentrouter.org/v1")
      expect(provider.options.apiKey).toBe("config-agentrouter-key")
...
test("provider.sort prioritizes preferred models", () => {
  ...
  expect(sorted[0].id).toBe("gpt-5.5-pro")
  expect(sorted[1].id).toBe("gpt-5.5")
...
test("adds openai gpt-5.5 fallback when models.dev is stale", () => {
  const database = {
  ...
  Provider.ensureOpenAIGPT55Fallback(database)

  const model = database[ProviderID.openai].models["gpt-5.5"]
  ...
  const pro = database[ProviderID.openai].models["gpt-5.5-pro"]
  expect(pro.name).toBe("GPT-5.5 Pro")
  expect(pro.api.id).toBe("gpt-5.5-pro")
  expect(pro.cost.input).toBe(30)
  expect(pro.cost.output).toBe(180)
  expect(pro.cost.cache.read).toBe(0)
...
test("plugin config providers persist after instance dispose", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const root = path.join(dir, ".zethcode", "plugin")
