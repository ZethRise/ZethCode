...
// patterns like `.zethcode/` in root .gitignore affect test results.
...
describe("file.ripgrep", () => {
    ...
    withTmpdirOutsideGit(async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          ...
          await Bun.write(path.join(dir, ".zethcode", "thing.json"), "{}")
        },
      })
      ...
      expect(files.includes(path.join(".zethcode", "thing.json"))).toBe(true)
    }))
  ...
  test("hidden false excludes hidden", async () => {
    ...
    expect(files.includes("visible.txt")).toBe(true)
    expect(files.includes(path.join(".zethcode", "thing.json"))).toBe(false)
  })
