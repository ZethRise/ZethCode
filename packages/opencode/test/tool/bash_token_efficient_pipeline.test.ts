...
describe("createPipeline", () => {
  ...
  test("ZETHCODE_BASH_RAW=1 bypasses the pipeline", () => {
    const prev = process.env.ZETHCODE_BASH_RAW
    process.env.ZETHCODE_BASH_RAW = "1"
    try {
      const drop: CleanPlugin = { name: "drop", apply: () => "" }
      const result = createPipeline([drop]).run("payload", { command: "ls" })
      expect(result.text).toBe("payload")
    } finally {
      if (prev === undefined) delete process.env.ZETHCODE_BASH_RAW
      else process.env.ZETHCODE_BASH_RAW = prev
    }
  })
