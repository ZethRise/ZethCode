...
for (const item of targets) {
  ...
  await Bun.build({
    ...
    compile: {
      ...
      outfile: `dist/${name}/bin/zeth`,
      execArgv: [`--user-agent=zethcode/${Script.version}`, "--use-system-ca", "--"],
      windows: {},
    },
    files: embeddedFileMap ? { "opencode-web-ui.gen.ts": embeddedFileMap } : {},
    entrypoints: ["./src/index.ts", parserWorker, workerPath, ...(embeddedFileMap ? ["opencode-web-ui.gen.ts"] : [])],
    define: {
      ZETHCODE_VERSION: `'${Script.version}'`,
      OPENCODE_MIGRATIONS: JSON.stringify(migrations),
      OTUI_TREE_SITTER_WORKER_PATH: bunfsRoot + workerRelativePath,
      OPENCODE_WORKER_PATH: workerPath,
      ZETHCODE_CHANNEL: `'${Script.channel}'`,
  ...
  if (item.os === process.platform && item.arch === process.arch && !item.abi) {
    const binaryPath = `dist/${name}/bin/zeth`
  ...
  await Bun.file(`dist/${name}/README.md`).write(
    `This is the ${item.os}-${item.arch} binary for [@zethrise/cli](https://www.npmjs.com/package/@zethrise/cli). Install that package directly.\n`,
  )
    ...
    JSON.stringify(
      {
        ...
        description: "Platform-specific binary for @zethrise/cli.",
        license: "MIT",
        author: "ZethRise",
        ...
        keywords: ["ai", "coding", "agent", "cli", "zethcode"],
        os: [item.os],
        cpu: [item.arch],
      },
      null,
