...
const PACKAGE_NAME = "@zethrise/cli"
...
export const USER_AGENT = `zethcode/${InstallationChannel}/${InstallationVersion}/${Flag.ZETHCODE_CLIENT}`
...
// TODO(zethcode): uncomment when corresponding channels are supported
...
export class Service extends Context.Service<Service, Interface>()("@zethcode/Installation") {}
...
export const layer: Layer.Layer<Service, never, HttpClient.HttpClient | ChildProcessSpawner.ChildProcessSpawner> =
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      // TODO(zethcode): uncomment when zethcode is published to homebrew
      ...
      const upgradeCurl = Effect.fnUntraced(
        function* (target: string) {
          ...
          const response = yield* httpOk.execute(HttpClientRequest.get(process.env.ZETHCODE_INSTALL_SCRIPT_URL ?? "https://mimo.xiaomi.com/install"))
      ...
      const upgradeCurlWindows = Effect.fnUntraced(function* (target: string) {
        ...
        const stageDir = path.join(os.tmpdir(), `zethcode_upgrade_${pid}`)

        // Download new version to staging dir (reuses install.ps1 logic)
        const installScriptUrl = process.env.ZETHCODE_INSTALL_SCRIPT_URL ?? "https://mimo.xiaomi.com/install.ps1"
        const downloadResult = yield* run(
          ...
          { env: { ZETHCODE_INSTALL_DIR: stageDir, VERSION: target, INSTALL_SCRIPT_URL: installScriptUrl } },
        )
      ...
      const methodImpl = Effect.fn("Installation.method")(function* () {
        if (process.execPath.includes(path.join(".zethcode", "bin"))) return "curl" as Method
        ...
        const checks: Array<{ name: Method; command: () => Effect.Effect<string> }> = [
          ...
          // TODO(zethcode): uncomment when zethcode is published to these channels
      ...
      const latestImpl = Effect.fn("Installation.latest")(function* (installMethod?: Method) {
        ...
        if (detectedMethod === "curl") {
          ...
          const base = (process.env.MIMO_FDS_BASE || "https://zethcode.cnbj1.mi-fds.com/zethcode/zethcode").replace(
            /\/+$/,
            "",
          )
        ...
        // TODO(zethcode): uncomment when zethcode is published to chocolatey
        ...
        // TODO(zethcode): uncomment when zethcode is published to scoop
        ...
        // TODO(zethcode): uncomment when zethcode has github releases
        // const response = yield* httpOk.execute(
      ...
      const upgradeImpl = Effect.fn("Installation.upgrade")(function* (m: Method, target: string) {
        ...
        if (!result || result.code !== 0) {
          // TODO(zethcode): restore choco-specific error when choco channel is supported
