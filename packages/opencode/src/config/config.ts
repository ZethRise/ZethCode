...
import { NamedError } from "@zethrise/shared/util/error"
...
function normalizeLoadedConfig(data: unknown, source: string) {
  ...
  delete copy.tui
  log.warn("tui keys in zethcode config are deprecated; move them to tui.json", { path: source })
  return copy
}
  ...
  memory: Schema.optional(
    Schema.Struct({
      cc_index: Schema.optional(Schema.Boolean).annotate({
        description:
          "Index Claude Code memory (~/.claude/projects/<slug>/memory) and expose under scope='cc'. Default: false. Note: when enabled, every zethcode agent (build/explore/subagents) can search these memories via the builtin `memory` tool — including CC's `type: user` (your role/preferences) and `type: feedback` (your guidance) categories. CC originally writes them for future CC sessions; flipping this on widens the consumer set to zethcode agents on the same machine. Leave disabled (default) if you don't want personal context recallable from a prompt-injection-vulnerable agent.",
      }),
    }),
  ),
...
function globalConfigFile() {
  const candidates = ["zethcode.jsonc", "zethcode.json", "config.json"].map((file) =>
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      let result: Info = pipe(
        ...
        mergeDeep(yield* loadFile(path.join(Global.Path.config, "zethcode.json"))),
        mergeDeep(yield* loadFile(path.join(Global.Path.config, "zethcode.jsonc"))),
      )
      ...
      const globalConfigFile = path.join(Global.Path.config, "zethcode.jsonc")
      if (
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "ZETHCODE_CONFIG_CONTENT") return "local"
        ...
        if (Flag.ZETHCODE_CONFIG) {
          yield* merge(Flag.ZETHCODE_CONFIG, yield* loadFile(Flag.ZETHCODE_CONFIG))
          log.debug("loaded custom config", { path: Flag.ZETHCODE_CONFIG })
        }

        if (!Flag.ZETHCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("zethcode", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.ZETHCODE_CONFIG_DIR) {
          log.debug("loading config from ZETHCODE_CONFIG_DIR", { path: Flag.ZETHCODE_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void, never>[] = []

        // Load Claude Code commands first so .zethcode commands override on name collision.
        for (const dir of yield* ConfigPaths.claudeCommandDirectories(ctx.directory, ctx.worktree)) {
          result.command = mergeDeep(result.command ?? {}, yield* Effect.promise(() => ConfigCommand.load(dir)))
        }

        for (const dir of directories) {
          if (dir.endsWith(".zethcode") || dir === Flag.ZETHCODE_CONFIG_DIR) {
            for (const file of ["zethcode.json", "zethcode.jsonc"]) {
          ...
          // Auto-discovered plugins under `.zethcode/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.ZETHCODE_CONFIG_CONTENT) {
          const source = "ZETHCODE_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.ZETHCODE_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          log.debug("loaded custom config from ZETHCODE_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["ZETHCODE_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("ZETHCODE_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (!Flag.ZETHCODE_DISABLE_CLAUDE_CODE_MCP) {
        ...
        if (Flag.ZETHCODE_DANGEROUSLY_SKIP_PERMISSIONS) {
        ...
        if (Flag.ZETHCODE_PERMISSION) {
          result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.ZETHCODE_PERMISSION))
        }
        ...
        if (Flag.ZETHCODE_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.ZETHCODE_DISABLE_PRUNE) {
