...
async function setupProjectIdEnvironment(workingDir: string): Promise<void> {
  ...
  const localFile = nodePath.join(workingDir, ".zethcode-project-id")
  const idFile = nodePath.join(mainGit, "zethcode-project-id")
  ...
  // Belt-and-suspenders: ensure .git/info/exclude lists .zethcode-project-id
  ...
  if (!existing.includes(".zethcode-project-id")) {
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const fakeVcs = Schema.decodeUnknownSync(Schema.optional(ProjectVcs))(Flag.ZETHCODE_FAKE_VCS)
    ...
    const fromDirectory = Effect.fn("Project.fromDirectory")(function* (directory: string) {
      ...
      const data: DiscoveryResult = yield* Effect.gen(function* () {
        ...
        // turn, which has shipped as a bug for users opening zethcode in $HOME.
      ...
      if (Flag.ZETHCODE_EXPERIMENTAL_ICON_DISCOVERY) yield* discover(existing).pipe(Effect.ignore, Effect.forkIn(scope))
