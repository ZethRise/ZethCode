 ...
 * TODO: lift to zethcode.json config (e.g. session.maxGoalReact).
 */
...
const OUTPUT_LENGTH_CONTINUATION_LIMIT = Flag.ZETHCODE_OUTPUT_LENGTH_CONTINUATION_LIMIT
const INVALID_OUTPUT_CONTINUATION_LIMIT = Flag.ZETHCODE_INVALID_OUTPUT_CONTINUATION_LIMIT
const TEXT_TOOL_CALL_RETRY_LIMIT = Flag.ZETHCODE_TEXT_TOOL_CALL_RETRY_LIMIT
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const predict = Effect.fn("SessionPrompt.predict")(function* (input: { sessionID: SessionID }) {
      ...
      const result = yield* Effect.tryPromise(() =>
        generateText({
          ...
          headers: {
            ...mdl.headers,
            "User-Agent": `zethcode/${InstallationVersion}`,
          },
          maxRetries: 1,
        }),
      ).pipe(
    ...
    const insertReminders = Effect.fn("SessionPrompt.insertReminders")(function* (input: {
      ...
      if (!Flag.ZETHCODE_DISABLE_BUILTIN_SKILLS && !Flag.ZETHCODE_DISABLE_OFFICIAL_SKILLS) {
...
NOTE: At any point in time through this workflow you should feel free to ask the user questions or clarifications. Don't make large assumptions about user intent. The goal is to present a well researched plan to the user, and tie any loose ends before implementation begins.
    ...
    const resolveTools = Effect.fn("SessionPrompt.resolveTools")(function* (input: {
      ...
      const askRouting = decideAskRouting({
        ...
        orchestratorEnabled: Flag.ZETHCODE_EXPERIMENTAL_ORCHESTRATOR,
      })
    ...
    const runLoop: (sessionID: SessionID, agentID?: string, task_id?: string) => Effect.Effect<MessageV2.WithParts> = Effect.fn(
      ...
      function* (sessionID: SessionID, agentID?: string, task_id?: string) {
        ...
        while (true) {
          ...
          // (explore/general/etc) spawned via zethcode's shared-sessionID
          ...
          if (step === 1 && !session.parentID) {
            ...
            const needAppRuntime = dreamTrigger || distillTrigger || Flag.ZETHCODE_EXPERIMENTAL_CRON
            if (needAppRuntime) {
              const { AppRuntime } = yield* Effect.promise(() => import("@/effect/app-runtime"))
              if (dreamTrigger) {
                AppRuntime.runPromise(
              ...
              // ZETHCODE_EXPERIMENTAL_CRON is unset; the outer gate just skips the
              ...
              if (Flag.ZETHCODE_EXPERIMENTAL_CRON) {
          ...
          const outcome: "break" | "continue" = yield* Effect.gen(function* () {
            ...
            // Context-Aware Automatic Memory Injection (Zero-Query Recall)
            const memoryAdditions: string[] = []
            const currentProjectId = resolveProjectId((yield* InstanceState.context).worktree)
            ...
            if (ftsQuery) {
              const sql = `
                SELECT memory_fts.path, memory_fts.scope
                FROM memory_fts_idx
                JOIN memory_fts ON memory_fts.id = memory_fts_idx.rowid
                WHERE memory_fts_idx MATCH ?
                  AND (memory_fts.scope = 'global' OR (memory_fts.scope = 'projects' AND memory_fts.scope_id = ?))
                ORDER BY bm25(memory_fts_idx)
                LIMIT 3
              `
              const rows = Database.Client().$client.query(sql).all(ftsQuery, currentProjectId) as { path: string; scope: string }[]
              if (rows.length > 0) {
                const bodies = yield* Effect.promise<string[]>(() =>
                  Promise.all(
                    rows.map(async (r) => {
                      const text = await Bun.file(r.path).text().catch(() => "")
                      if (!text.trim()) return ""
                      return `### ${path.basename(r.path)} (${r.scope})\n${text}`
                    })
                  )
                )
                const validBodies = bodies.filter(Boolean)
                if (validBodies.length > 0) {
                  memoryAdditions.push(
                    "<recalled-memories>",
                    "The following rules/instructions were automatically retrieved from project/global memory based on relevance:",
                    ...validBodies,
                    "</recalled-memories>"
                  )
                }
 ...
 * (zethcode convention for `isMeta`) so transcript-preview surfaces can hide it,
