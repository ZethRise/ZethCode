  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const relative = Effect.fnUntraced(function* (instruction: string) {
        ...
        if (!Flag.ZETHCODE_CONFIG_DIR) {
          log.warn(
            `Skipping relative instruction "${instruction}" - no ZETHCODE_CONFIG_DIR set while project config is disabled`,
          )
          return []
        }
        return yield* fs
          .globUp(instruction, Flag.ZETHCODE_CONFIG_DIR, Flag.ZETHCODE_CONFIG_DIR)
          .pipe(Effect.catch(() => Effect.succeed([] as string[])))
      })
