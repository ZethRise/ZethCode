...
import { Slug } from "@zethrise/shared/util/slug"
...
export async function run(opts?: { force?: boolean }) {
  ...
  for (const file of files) {
    ...
    try {
      ...
      // user deleted it in zethcode, drop the stale mapping and import fresh —
      ...
      Database.transaction((tx) => {
        ...
        if (existing) {
          ...
          // user's zethcode-native continuation messages in the same session.
          ...
          // Preserve zethcode-owned metadata on re-sync: keep any user rename
