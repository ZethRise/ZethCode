...
const DEFAULT_TIMEOUT = Flag.ZETHCODE_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS || 2 * 60 * 1000
...
export const BashTool = Tool.define(
  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("BashTool.run")(function* (
      ...
      const cleaned =
        !file && Flag.ZETHCODE_EXPERIMENTAL_TOKEN_EFFICIENCY
      ...
      const heuristic =
        !file &&
        Flag.ZETHCODE_EXPERIMENTAL_TOKEN_EFFICIENCY &&
        Flag.ZETHCODE_EXPERIMENTAL_TOKEN_EFFICIENCY_HEURISTIC
        ...
        return {
          ...
          execute: (params: z.infer<typeof Parameters>, ctx: Tool.Context) =>
            Effect.gen(function* () {
              ...
              // ZETHCODE_AUTO_APPROVE_DELETE trusts deletes and falls back to
              // the regular ask (where a `bash: deny` rule still blocks).
              if (scan.deletes.size > 0 && !Flag.ZETHCODE_AUTO_APPROVE_DELETE) {
