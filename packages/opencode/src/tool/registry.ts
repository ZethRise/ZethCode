...
import { type ToolContext as PluginToolContext, type ToolDefinition } from "@zethrise/plugin"
import z from "zod"
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("ToolRegistry.state")(function* (ctx) {
        ...
        const questionEnabled =
          ["app", "cli", "desktop"].includes(Flag.ZETHCODE_CLIENT) || Flag.ZETHCODE_ENABLE_QUESTION_TOOL
        ...
        return {
          ...
          builtin: [
            ...
            tool.patch,
            tool.changedir,
            ...(Flag.ZETHCODE_EXPERIMENTAL_LSP_TOOL ? [tool.lsp] : []),
            tool.planexit,
            tool.planenter,
            tool.memory,
            tool.history,
            tool.task,
            ...(Flag.ZETHCODE_EXPERIMENTAL_CRON ? [tool.cron] : []),
            ...(Flag.ZETHCODE_EXPERIMENTAL_ORCHESTRATOR ? [tool.session] : []),
            ...(Flag.ZETHCODE_EXPERIMENTAL_WORKFLOW_TOOL ? [tool.workflow] : []),
          ],
          actor: tool.actor,
          read: tool.read,
        }
        ...
        if (tool.id === CodeSearchTool.id || tool.id === WebSearchTool.id) {
          if (tool.id === WebSearchTool.id) {
            return (
              ...
              Flag.ZETHCODE_ENABLE_EXA
            )
          }
          return input.providerID === ProviderID.opencode || Flag.ZETHCODE_ENABLE_EXA
        }
