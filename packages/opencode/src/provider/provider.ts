...
import { Npm } from "../npm"
import { Hash } from "@zethrise/shared/util/hash"
...
// per-provider via zethcode.json's `chunkTimeout` config for tighter or looser
// bounds.
  ...
  return {
    ...
    "amazon-bedrock": Effect.fnUntraced(function* () {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>) {
          ...
          // 1. options.region from zethcode.json provider config
    ...
    agentrouter: Effect.fnUntraced(function* (input: Info) {
      ...
      const apiKey = iife(() => {
        ...
        return env["AGENTROUTER_API_KEY"] ?? env["AGENT_ROUTER_TOKEN"]
      })
      return {
    ...
    gitlab: Effect.fnUntraced(function* (input: Info) {
      ...
      const aiGatewayHeaders = {
        "User-Agent": `zethcode/${InstallationVersion} gitlab-ai-provider/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`,
    ...
    "cloudflare-workers-ai": Effect.fnUntraced(function* (input: Info) {
      ...
      return {
        ...
        options: {
          apiKey,
          headers: {
            "User-Agent": `zethcode/${InstallationVersion} cloudflare-workers-ai (${os.platform()} ${os.release()}; ${os.arch()})`,
          },
        },
        async getModel(sdk: any, modelID: string) {
          return sdk.languageModel(modelID)
    ...
    "cloudflare-ai-gateway": Effect.fnUntraced(function* (input: Info) {
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `zethcode auth cloudflare-ai-gateway`.",
        )
      }
      ...
      const opts = {
        ...
        headers: {
          "User-Agent": `zethcode/${InstallationVersion} cloudflare-ai-gateway (${os.platform()} ${os.release()}; ${os.arch()})`,
        },
      }
    ...
    cerebras: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "zethcode",
          },
        },
      }),
    kilo: () =>
...
export function ensureOpenAIGPT55Fallback(database: Record<string, Info>) {
  const openai = database[ProviderID.openai]
  if (!openai) return

  const fallback = openai.models["gpt-5.4"] ?? openai.models["gpt-5"]
  if (fallback && !openai.models["gpt-5.5"]) {
    const model: Model = {
      ...fallback,
      ...
      cost: {
        ...
        output: 30,
        cache: {
          read: fallback.cost.cache.read,
          write: fallback.cost.cache.write,
        },
      },
      limit: {
        context: 1_000_000,
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(() =>
      Effect.gen(function* () {
        ...
        ensureOpenAIGPT55Fallback(database)
                ...
                context: (() => {
                  ...
                  if (!warnedContextDefaults.has(key)) {
                    ...
                    log.warn("limit.context not configured and not found in models.dev", {
                      ...
                      fix: `Set limit.context explicitly in zethcode.json under provider.${providerID}.models.${modelID}`,
                    })
                  }
        ...
        if (!Flag.ZETHCODE_DISABLE_PROVIDER_ENV) {
        ...
        for (const [id, provider] of Object.entries(providers)) {
          ...
          for (const [modelID, model] of Object.entries(provider.models)) {
            ...
            if (model.status === "alpha" && !Flag.ZETHCODE_ENABLE_EXPERIMENTAL_MODELS) delete provider.models[modelID]
    ...
    async function resolveSDK(model: Model, s: State, envs: Record<string, string | undefined>) {
      try {
        ...
        options["fetch"] = async (input: any, init?: BunFetchRequestInit) => {
          ...
          const opts = init ?? {}
          if (model.providerID === "agentrouter") {
            const h = new Headers(opts.headers)
            const ua = h.get("User-Agent")
            if (ua && ua.includes("zethcode")) {
              h.set("User-Agent", ua.replace("zethcode", "opencode"))
              opts.headers = h
            } else if (!ua) {
...
const priority = ["gpt-5", "claude-sonnet-4", "big-pickle", "gemini-3-pro", "gpt-5.5", "gpt-5.5-pro"]
export function sort<T extends { id: string }>(models: T[]) {
  return sortBy(
    models,
    [(model) => priority.reduce((result, filter, index) => (model.id.includes(filter) ? index : result), -1), "desc"],
