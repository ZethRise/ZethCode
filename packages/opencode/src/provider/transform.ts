...
export const OUTPUT_TOKEN_MAX = Flag.ZETHCODE_EXPERIMENTAL_OUTPUT_TOKEN_MAX || 32_000
...
function limitImages(msgs: ModelMessage[]): ModelMessage[] {
  const maxImages = Flag.ZETHCODE_MAX_PROMPT_IMAGES
  const maxSize = Flag.ZETHCODE_MAX_PROMPT_IMAGE_SIZE
...
export function variants(model: Provider.Model): Record<string, Record<string, any>> {
  ...
  switch (model.api.npm) {
    ...
    case "@ai-sdk/openai":
      ...
      if (id === "gpt-5.5-pro") {
        return Object.fromEntries(["medium", "high", "xhigh"].map((effort) => [effort, { reasoningEffort: effort }]))
      }
...
export function options(input: {
  ...
  if (input.model.api.id.includes("gpt-5") && !input.model.api.id.includes("gpt-5-chat")) {
    if (!input.model.api.id.includes("-pro")) {
