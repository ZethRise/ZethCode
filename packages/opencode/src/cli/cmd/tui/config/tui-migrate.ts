...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from zethcode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const zethcode = await zethcodeFiles(input)
  for (const file of zethcode) {
...
async function zethcodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "zethcode"),
    ...(await Filesystem.findUp(["zethcode.json", "zethcode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "zethcode"))
  }
  if (Flag.ZETHCODE_CONFIG) files.push(Flag.ZETHCODE_CONFIG)
