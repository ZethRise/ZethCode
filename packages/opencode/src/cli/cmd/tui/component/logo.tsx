...
export function Logo(props: { shape?: LogoShape; ink?: RGBA; idle?: boolean; sweep?: boolean } = {}) {
  ...
  const ZETH_BLUE = RGBA.fromInts(37, 99, 235)
  const ZETH_LIGHT_BLUE = RGBA.fromInts(147, 197, 253)
          ...
          if (labelRow) {
            return (
              <box flexDirection="row" gap={1}>
                <text fg={ZETH_LIGHT_BLUE} selectable={false}>{line}</text>
                <text fg={ZETH_LIGHT_BLUE} selectable={false}>{ctx.shape.right[index()]}</text>
              </box>
            )
          }
          return (
            <box flexDirection="row" gap={1}>
              <box flexDirection="row">
                {renderLine(line, index(), props.ink ?? ZETH_BLUE, true, 0, frame(), dusk(), idleState())}
              </box>
              <box flexDirection="row">
                {renderLine(
                  ctx.shape.right[index()],
                  index(),
                  props.ink ?? ZETH_LIGHT_BLUE,
                  true,
                  ctx.LEFT + GAP,
                  frame(),
                  dusk(),
