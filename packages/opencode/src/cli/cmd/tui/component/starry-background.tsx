    ...
    f.grid.forEach((row, y) => {
      row.forEach((cell, x) => {
        ...
        const baseColor = dark
          ? tint(theme.background, RGBA.fromInts(170, 215, 255), Math.min(1, b * 1.05))
          : tint(theme.background, RGBA.fromInts(70, 130, 180), Math.min(1, b * 0.95))
        const starColor =
          peak > 0
            ? tint(baseColor, dark ? RGBA.fromInts(255, 255, 255) : RGBA.fromInts(30, 60, 100), peak * (dark ? 0.65 : 0.5))
            : baseColor
