...
import type { GlobalEvent } from "@zethrise/sdk/v2"
...
export const TuiThreadCommand = cmd({
  command: "$0 [project]",
  describe: "start zethcode tui",
  builder: (yargs) =>
    withNetworkOptions(yargs)
      .positional("project", {
        type: "string",
        describe: "path to start zethcode in",
      })
      .option("model", {
        type: "string",
        alias: ["m"],
  ...
  handler: async (args) => {
    ...
    try {
      ...
      if (args["dangerously-skip-permissions"]) {
        ...
        process.env.ZETHCODE_DANGEROUSLY_SKIP_PERMISSIONS = "1"
      }

      const env = sanitizedProcessEnv({
        [ZETHCODE_PROCESS_ROLE]: "worker",
        [ZETHCODE_RUN_ID]: ensureRunID(),
      })
