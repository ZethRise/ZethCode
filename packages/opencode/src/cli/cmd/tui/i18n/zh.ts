...
export const dict = {
  ...
  "tui.home.agreement.prefix": "使用 Zeth Code 即表示你同意我们的",
  ...
  "tui.tips.config_files":
    "创建 {highlight}zethcode.json{/highlight} 用于服务端配置，{highlight}tui.json{/highlight} 用于 TUI 配置",
  "tui.tips.global_config": "把 TUI 配置放在 {highlight}~/.config/zethcode/tui.json{/highlight} 作为全局配置",
  ...
  "tui.tips.mcp_oauth": "Zeth Code 自动处理需要鉴权的远程 MCP 服务器的 OAuth",
  "tui.tips.custom_command":
    "在 {highlight}.zethcode/command/{/highlight} 添加 {highlight}.md{/highlight} 文件以定义可复用自定义提示",
  ...
  "tui.tips.custom_agent":
    "在 {highlight}.zethcode/agent/{/highlight} 添加 {highlight}.md{/highlight} 文件来创建专用 AI 角色",
  ...
  "tui.tips.formatter": "Zeth Code 自动使用 prettier、gofmt、ruff 等格式化文件",
  ...
  "tui.tips.lsp": "Zeth Code 使用 LSP 服务器进行智能代码分析",
  "tui.tips.custom_tool":
    "在 {highlight}.zethcode/tools/{/highlight} 创建 {highlight}.ts{/highlight} 文件定义新的 LLM 工具",
  "tui.tips.tool_scripts": "工具定义可以调用 Python、Go 等脚本",
  "tui.tips.plugins":
    "在 {highlight}.zethcode/plugin/{/highlight} 添加 {highlight}.ts{/highlight} 文件挂接事件钩子",
  "tui.tips.plugin_notify": "用插件在会话完成时发送系统通知",
  "tui.tips.plugin_protect": "写一个插件阻止 Zeth Code 读取敏感文件",
  ...
  "tui.tips.serve": "运行 {highlight}mimo serve{/highlight} 以无头模式提供 Zeth Code API",
  ...
  "tui.tips.theme_files": "在 {highlight}.zethcode/themes/{/highlight} 目录下创建 JSON 主题文件",
  ...
  "tui.tips.zen": "搭配 Zeth Code 使用 {highlight}/connect{/highlight} 选用经过测试的精选模型",
  ...
  "tui.skill.zethcode.description": "Zeth Code 功能、配置与命令的自文档参考",
  ...
  "tui.toast.update_available.success": "已更新到 Zeth Code v{{version}}，请重启应用。",
  ...
  "trust.capabilities": "Zeth Code 将能够读取、编辑和执行此目录中的文件。",
  ...
  "skip_permissions.body":
    "你使用 --dangerously-skip-permissions 启动。Zeth Code 将在不征求同意的情况下读取、修改、执行文件并运行 Shell 命令。只有你在配置中明确拒绝（deny）的规则仍然生效。由此产生的一切后果由你自行承担。",
