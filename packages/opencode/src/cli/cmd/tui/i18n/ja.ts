...
export const dict = {
  ...
  "tui.home.agreement.prefix": "Zeth Code をご利用いただくことで、",
  ...
  "tui.tips.config_files":
    "サーバ設定は {highlight}zethcode.json{/highlight}、TUI 設定は {highlight}tui.json{/highlight} を作成します",
  "tui.tips.global_config":
    "TUI 設定をグローバルに置くなら {highlight}~/.config/zethcode/tui.json{/highlight} を使います",
  ...
  "tui.tips.mcp_oauth": "Zeth Code は認証が必要なリモート MCP サーバの OAuth を自動処理します",
  "tui.tips.custom_command":
    "{highlight}.zethcode/command/{/highlight} に {highlight}.md{/highlight} ファイルを追加すると再利用可能なカスタムプロンプトを定義できます",
  ...
  "tui.tips.custom_agent":
    "{highlight}.zethcode/agent/{/highlight} に {highlight}.md{/highlight} ファイルを追加して専用 AI ペルソナを作れます",
  ...
  "tui.tips.formatter": "Zeth Code は prettier、gofmt、ruff などでファイルを自動整形します",
  ...
  "tui.tips.lsp": "Zeth Code はインテリジェントなコード解析に LSP サーバを使います",
  "tui.tips.custom_tool":
    "{highlight}.zethcode/tools/{/highlight} に {highlight}.ts{/highlight} ファイルを作って新しい LLM ツールを定義します",
  "tui.tips.tool_scripts": "ツール定義から Python、Go などのスクリプトを呼び出せます",
  "tui.tips.plugins":
    "{highlight}.zethcode/plugin/{/highlight} に {highlight}.ts{/highlight} ファイルを追加してイベントフックを実装できます",
  "tui.tips.plugin_notify": "プラグインでセッション完了時に OS 通知を送れます",
  "tui.tips.plugin_protect": "プラグインを書いて Zeth Code が機密ファイルを読まないようにできます",
  ...
  "tui.tips.serve": "{highlight}mimo serve{/highlight} を実行するとヘッドレスで Zeth Code API にアクセスできます",
  ...
  "tui.tips.theme_files": "{highlight}.zethcode/themes/{/highlight} ディレクトリに JSON テーマファイルを作成します",
  ...
  "tui.tips.zen": "Zeth Code と組み合わせて {highlight}/connect{/highlight} で厳選されたモデルを使えます",
  ...
  "tui.skill.zethcode.description": "Zeth Code の機能・設定・コマンドに関するセルフドキュメント",
  ...
  "tui.toast.update_available.success": "Zeth Code v{{version}} に更新しました。アプリケーションを再起動してください。",
  ...
  "trust.capabilities": "Zeth Code はここでファイルの読み取り、編集、実行が可能になります。",
  ...
  "skip_permissions.body":
    "--dangerously-skip-permissions を指定して起動しました。Zeth Code は承認を求めずにファイルの読み取り・編集・実行やシェルコマンドの実行を行います。設定で明示的に拒否（deny）したルールのみが引き続き適用されます。発生したいかなる結果についてもあなた自身が全責任を負います。",
