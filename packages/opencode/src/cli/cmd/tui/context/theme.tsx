...
import zethcode from "./theme/zethcode.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@zethrise/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light"; plain?: boolean }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = props.plain ? "system" : config.theme ?? kv.get("theme", "zethcode")
        draft.active = typeof active === "string" ? active : "zethcode"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.zethcode, store.mode)
    })
...
function generateSystem(colors: TerminalColors, mode: "dark" | "light"): ThemeJson {
  ...
  const zethBlue = RGBA.fromInts(37, 99, 235)

  return {
    theme: {
      // Primary colors using Zeth blue
      primary: zethBlue,
      secondary: zethBlue,
      accent: zethBlue,
