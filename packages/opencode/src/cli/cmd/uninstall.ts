...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall zethcode and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall Zeth Code")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g @zethrise/cli",
      pnpm: "pnpm uninstall -g @zethrise/cli",
      bun: "bun remove -g @zethrise/cli",
      // TODO(zethcode): uncomment when published to these channels
      // brew: "brew uninstall zethcode",
      // choco: "choco uninstall zethcode",
      // scoop: "scoop uninstall zethcode",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using Zeth Code!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# zethcode") || content.includes(".zethcode/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".zethcode/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".zethcode/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".zethcode"))
    ) {
      continue
    }
...
async function cleanWindowsPath() {
  const installDir = path.join(os.homedir(), ".zethcode", "bin")
  const script = `
    $installDir = $env:ZETHCODE_UNINSTALL_DIR
  ...
  const result = await Process.run(["powershell", "-ep", "Bypass", "-c", script], {
    nothrow: true,
    env: { ...process.env, ZETHCODE_UNINSTALL_DIR: installDir },
  })
