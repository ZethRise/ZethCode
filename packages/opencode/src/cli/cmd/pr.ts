...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run zethcode",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for zethcode session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found zethcode session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["zeth", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting zethcode...")
        UI.println()

        const mimoArgs = sessionId ? ["-s", sessionId] : []
        const mimoProcess = Process.spawn(["zeth", ...mimoArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        ...
        if (code !== 0) throw new Error(`zeth exited with code ${code}`)
      },
    })
  },
})
