...
import { createOpencodeClient, type OpencodeClient, type ToolPart } from "@zethrise/sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run zethcode with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running zethcode server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to ZETHCODE_SERVER_PASSWORD)",
      })
      .option("dir", {
        type: "string",
  ...
  handler: async (args) => {
    ...
    async function share(sdk: OpencodeClient, sessionID: string) {
      ...
      if (!cfg.data) return
      if (cfg.data.share !== "auto" && !Flag.ZETHCODE_AUTO_SHARE && !args.share) return
    ...
    if (args.attach) {
      const headers = (() => {
        const password = args.password ?? process.env.ZETHCODE_SERVER_PASSWORD
        if (!password) return undefined
        const username = process.env.ZETHCODE_SERVER_USERNAME ?? "zethcode"
