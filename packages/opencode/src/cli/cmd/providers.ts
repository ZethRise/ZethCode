...
import type { Hooks } from "@zethrise/plugin"
...
export const ProvidersLoginCommand = cmd({
  ...
  builder: (yargs) =>
    yargs
      .positional("url", {
        describe: "zethcode auth provider",
        type: "string",
      })
      .option("provider", {
        alias: ["p"],
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (provider === "other") {
          ...
          prompts.log.warn(
            `This only stores a credential for ${provider} - you will need configure it in zethcode.json, check the docs for examples.`,
          )
        }
        ...
        if (provider === "amazon-bedrock") {
          prompts.log.info(
            "Amazon Bedrock authentication priority:\n" +
              ...
              "Configure via zethcode.json options (profile, region, endpoint) or\n" +
