...
import { ConnectorCommand } from "./cli/cmd/connector"
...
const cli = yargs(args)
  ...
  .middleware(async (opts) => {
    ...
    process.env.AGENT = "1"
    process.env.ZETHCODE = "1"
    process.env.ZETHCODE_PID = String(process.pid)

    Log.Default.info("zethcode", {
    ...
    const marker = path.join(Global.Path.data, "zethcode.db")
    ...
    if (!process.env.ZETHCODE_DISABLE_CLAUDE_IMPORT && !process.env.ZETHCODE_CLAUDE_IMPORTED) {
      process.env.ZETHCODE_CLAUDE_IMPORTED = "1"
      try {
