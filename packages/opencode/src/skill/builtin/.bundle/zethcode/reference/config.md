# ZethCode Configuration Reference

## File locations & precedence

Config is JSON or JSONC. ZethCode discovers it by walking up from the cwd to the worktree root, then falls back to global.

- Project: `.zethcode/zethcode.json` or `.zethcode/zethcode.jsonc`
- Global: `~/.config/zethcode/zethcode.json` (XDG config dir)
- Extra config dirs are also searched via `$ZETHCODE_CONFIG_DIR`.

Project config merges **over** global. Always include `"$schema": "https://mimo.xiaomi.com/zethcode/config.json"` for validation.

## On-disk data layout

Base directories resolve from `ZETHCODE_HOME` if set (must be absolute → `<home>/{data,cache,config,state}`), otherwise from XDG:

| Kind | Default location | Holds |
|------|------------------|-------|
| data | `~/.local/share/zethcode/` | memory, logs, `builtin_skills/<version>/`, bin |
| config | `~/.config/zethcode/` | global `zethcode.json` |
| cache | `~/.cache/zethcode/` | caches, downloaded bins |
| state | `~/.local/state/zethcode/` | runtime state |

Memory files live under `~/.local/share/zethcode/memory/`:
...
- `ZETHCODE_HOME` — override all base dirs (absolute path).
- `ZETHCODE_CONFIG_DIR` — extra config directory to search.
- `ZETHCODE_PURE` — run without external plugins (same as `mimo --pure`). Does **not** change models or Claude Code inheritance.
- `ZETHCODE_MIMO_ONLY` — pure-MiMo mode: don't inherit Claude Code settings (CLAUDE.md, `~/.claude/skills`), don't read provider API keys from env, fall back to the mimo-auto model.
- `ZETHCODE_DISABLE_LOG_ROTATION` — keep a single growing log file instead of rotating.
- `ZETHCODE_TEXT_TOOL_CALL_RETRY_LIMIT` — retries when a model emits a tool call as prose markup instead of a structured call (default 2).
- `ZETHCODE_EXPERIMENTAL_CRON` — scheduled prompts (cron/loop); **on by default**. `ZETHCODE_DISABLE_CRON` kills it at runtime. Tune loop keepalive with `ZETHCODE_LOOP_KEEPALIVE_BUDGET` (default 1) and `ZETHCODE_LOOP_KEEPALIVE_DELAY_S` (default 1200).
- `ZETHCODE_EXPERIMENTAL_TOKEN_EFFICIENCY_HEURISTIC` — shape-based compaction of bash output to save tokens; off by default.
- `ZETHCODE_DISABLE_BUILTIN_SKILLS`, `_COMPOSE_SKILLS`, `_EXTERNAL_SKILLS`, `_CLAUDE_CODE_SKILLS`, `_CODEX_SKILLS`, `_OPENCODE_SKILLS`, `_PROJECT_CONFIG`, `_CLAUDE_IMPORT` — feature toggles.
...
- A ref without `/` is a group name. If configured, ZethCode is **provider-aware**: it prefers a member on the caller's current provider, otherwise falls back to the group's `default`.
...
Prefer a markdown file (`.zethcode/agent/<name>.md`, body = system prompt) for defining a custom agent/mode — see the "Custom agents & modes" section in @guide.md. Use the `agent` config key for short, inline per-agent overrides.
