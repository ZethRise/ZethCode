...
2. **Pick a model** — set `"model": "provider/model"` in config, or switch live in the TUI model dialog. Provider API keys are auto-detected from environment variables (unless `ZETHCODE_MIMO_ONLY=1`).
3. **List what's available** — `mimo models`, `mimo providers`.

## Memory: making ZethCode remember
...
Drop a markdown file at `.zethcode/command/<name>.md` (or `.zethcode/commands/`, `.claude/command(s)/` are also read). The frontmatter configures it; the body is the prompt template.
...
A "mode" is just a **primary agent** with its own system prompt. To give ZethCode a custom mode (e.g. a `general` chat mode alongside the coding-focused `build`), drop a markdown file — no code, no server changes. The frontmatter is config; the **markdown body becomes the agent's system prompt**.
...
| Path | Scope |
|------|-------|
| `.zethcode/agent/<name>.md` (or `agents/`) | project agent — most common |
| `.zethcode/mode/<name>.md` (or `modes/`) | project mode — same as an agent forced to `mode: primary` |
| `~/.config/zethcode/agent/<name>.md` | global agent, available in every project |
...
**Cron** schedules a prompt to be injected into a session on a recurring or one-shot basis. It is driven by the `cron` tool (no `/cron` slash command); `ZETHCODE_EXPERIMENTAL_CRON` is **on by default** (kill switch: `ZETHCODE_DISABLE_CRON`).
...
- **Durable** jobs persist to `<project>/.zethcode/scheduled_tasks.json` and survive restart; non-durable jobs live only for the session.
...
To add tools, hooks, or skills, use the `evolve` skill — it covers writing `.zethcode/tools/*.ts`, `.zethcode/hooks/*.ts`, and `.zethcode/skills/*/SKILL.md`, all hot-reloaded on the next turn.
