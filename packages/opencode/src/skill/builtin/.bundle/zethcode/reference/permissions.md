# ZethCode Permissions Reference
...
| Any surface (env) | `ZETHCODE_PERMISSION='"allow"'` or `ZETHCODE_DANGEROUSLY_SKIP_PERMISSIONS=1` |
...
- `external_directory` governs reads/writes outside the project working directory — by default these prompt, so ZethCode never silently widens scope.
