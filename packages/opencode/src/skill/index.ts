...
import { Glob } from "@zethrise/shared/util/glob"
...
const ZETHCODE_SKILL_PATTERN = "{skill,skills}/**/SKILL.md"
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  if (!Flag.ZETHCODE_DISABLE_BUILTIN_SKILLS) {
    ...
    if (builtinSkillRoot && (yield* fsys.isDir(builtinSkillRoot))) {
      ...
      if (Flag.ZETHCODE_DISABLE_OFFICIAL_SKILLS) {
  ...
  if (!Flag.ZETHCODE_DISABLE_COMPOSE_SKILLS) {
  ...
  if (!Flag.ZETHCODE_DISABLE_EXTERNAL_SKILLS) {
    const externalDirs = EXTERNAL_DIRS.filter((dir) => {
      if (dir === ".claude" && Flag.ZETHCODE_DISABLE_CLAUDE_CODE_SKILLS) return false
      if (dir === ".codex" && Flag.ZETHCODE_DISABLE_CODEX_SKILLS) return false
      if (dir === ".opencode" && Flag.ZETHCODE_DISABLE_OPENCODE_SKILLS) return false
      return true
    })
  ...
  for (const dir of configDirs) {
    yield* scan(state, dir, ZETHCODE_SKILL_PATTERN)
  }
