...
function create(opts: { cors?: string[] }) {
  ...
  if (Flag.ZETHCODE_WORKSPACE_ID) {
    return {
      app: app
        .use(InstanceMiddleware(Flag.ZETHCODE_WORKSPACE_ID ? WorkspaceID.make(Flag.ZETHCODE_WORKSPACE_ID) : undefined))
        .use(FenceMiddleware)
...
export async function listen(opts: {
  ...
  if (!isLoopback && !Flag.ZETHCODE_SERVER_PASSWORD && !opts.noAuth) {
    throw new Error(
      "Refusing to bind to non-loopback address without ZETHCODE_SERVER_PASSWORD. " +
