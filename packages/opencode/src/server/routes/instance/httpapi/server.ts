...
const Headers = Schema.Struct({
  ...
  "x-zethcode-directory": Schema.optional(Schema.String),
})
...
const auth = Layer.succeed(
  ...
  Authorization.of({
    basic: (effect, { credential }) =>
      Effect.gen(function* () {
        if (!Flag.ZETHCODE_SERVER_PASSWORD) return yield* effect

        const user = Flag.ZETHCODE_SERVER_USERNAME ?? "zethcode"
        if (credential.username !== user) {
          return yield* new Unauthorized({ message: "Unauthorized" })
        }
        if (Redacted.value(credential.password) !== Flag.ZETHCODE_SERVER_PASSWORD) {
          return yield* new Unauthorized({ message: "Unauthorized" })
        }
        return yield* effect
      }),
  ...
  Effect.gen(function* () {
    return (effect) =>
      Effect.gen(function* () {
        ...
        const raw = query.directory || headers["x-zethcode-directory"] || process.cwd()
