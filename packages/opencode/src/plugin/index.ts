...
import { createOpencodeClient } from "@zethrise/sdk"
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("Plugin.state")(function* (ctx) {
        ...
        const client = createOpencodeClient({
          ...
          headers: Flag.ZETHCODE_SERVER_PASSWORD
            ? {
                Authorization: `Basic ${Buffer.from(`${Flag.ZETHCODE_SERVER_USERNAME ?? "zethcode"}:${Flag.ZETHCODE_SERVER_PASSWORD}`).toString("base64")}`,
              }
            : undefined,
        ...
        const plugins = Flag.ZETHCODE_PURE ? [] : (cfg.plugin_origins ?? [])
        if (Flag.ZETHCODE_PURE && cfg.plugin_origins?.length) {
