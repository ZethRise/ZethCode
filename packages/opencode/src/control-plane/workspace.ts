...
export const create = fn(CreateInput, async (input) => {
  ...
  const env = {
    ZETHCODE_AUTH_CONTENT: JSON.stringify(await AppRuntime.runPromise(Auth.Service.use((auth) => auth.all()))),
    ZETHCODE_WORKSPACE_ID: config.id,
    ZETHCODE_EXPERIMENTAL_WORKSPACES: "true",
...
async function startSync(space: Info) {
  if (!Flag.ZETHCODE_EXPERIMENTAL_WORKSPACES) return
