...
/**
 * Resolve zethcode's four base directories (config/data/state/cache)
 * from environment variables.
 *
 * If ZETHCODE_HOME is set and non-empty, the four paths are subdirectories
 * of it. Otherwise, falls through to XDG Base Directory defaults.
 *
 * @throws if ZETHCODE_HOME is set but not an absolute path
 */
export function resolveMimocodeHome(env: NodeJS.ProcessEnv = process.env): ResolvedPaths {
  const home = env.ZETHCODE_HOME
  if (home) {
    if (!path.isAbsolute(home)) {
      throw new Error(
        `ZETHCODE_HOME must be an absolute path, got: ${JSON.stringify(home)}`,
      )
    }
    return {
      mode: "zethcode_home",
